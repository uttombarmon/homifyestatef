    ...
    return (
        <>
            <tr>
                ...
                <td>{name}</td>
                <td>{email}</td>
                <td><button onClick={()=>makeAdmin(d._id)} className={ `btn btn-info ${role === 'fraud' || role === 'admin' && 'btn-disabled'}`}>Make Admin</button></td>
                ...
                <td><button onClick={()=>handleDelete(d._id)} className=" btn btn-error">Delete</button></td>
            </tr>
        </>
    );
}
